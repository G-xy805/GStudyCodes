...
| 步骤 | 代码语句 | 调用函数 | char('a')内存引用计数 | char('b')内存引用计数 | 操作说明 |
|------|----------|----------|----------------------|----------------------|----------|
...
| 6 | `cp3 = cp3` | 赋值操作符 | 3 | - | cp3自我赋值，无变化，引用计数=3 |
| 7 | `SmartPointer<char> cp4(new char('b'))` | 构造函数 | 3 | 1 | 创建cp4，新内存char('b')，引用计数=1 |
| 8 | `cp3 = cp4` | 赋值操作符 | 2 | 2 | cp3切换到cp4内存，char('a')计数减1，char('b')计数加1 |
| 9 | 主函数结束 | 析构函数 | 2 | 1 | 函数结束，开始析构对象 |
| 10 | 析构cp4 | 析构函数 | 2 | 0 | 析构cp4，char('b')引用计数减1，为0则释放内存 |
| 11 | 析构cp3 | 析构函数 | 1 | - | 析构cp3，char('b')已释放，char('a')引用计数减1 |
| 12 | 析构cp2 | 析构函数 | 1 | - | 析构cp2，char('a')引用计数仍为1 |
| 13 | 析构cp1 | 析构函数 | 0 | - | 析构cp1，char('a')引用计数减1，为0则释放内存 |
