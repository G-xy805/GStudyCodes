---
title: 设计模式——享元模式
description: 详细介绍享元模式的概念、优缺点及C++实现方式
pubDate: 2025-12-06
image: "/image/flyweight-pattern.jpg"
categories:
  - 设计模式
tags:
  - 享元模式
...
public:
    UnsharedConcreteFlyweight(const std::string& info) {
        this->info = info;
    }
...
```cpp
class UnsharedConcreteFlyweight; // 前向声明
...
public:
    ConcreteFlyweight(const std::string& key) {
        this->key = key;
...
private:
    std::unordered_map<std::string, std::shared_ptr<Flyweight>> flyweightMap;

public:
    std::shared_ptr<Flyweight> getFlyweight(const std::string& key) {
        auto it = flyweightMap.find(key);
...
int main() {
    ...
    auto f01 = factory.getFlyweight("a");
    auto f02 = factory.getFlyweight("a");
    auto f11 = factory.getFlyweight("b");
    auto f12 = factory.getFlyweight("b");
