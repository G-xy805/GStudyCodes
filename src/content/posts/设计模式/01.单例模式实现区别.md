---
title: 单例模式实现方式的区别
description: 详细介绍单例模式的各种实现方式及其优缺点，包括饿汉模式、懒汉模式、双重检查锁等
pubDate: 2025-12-06
image: "/image/singleton-pattern.jpg"
categories:
  - 设计模式
tags:
  - 单例模式
...
class Wife1 {
private:
    static const std::unique_ptr<Wife1> wife;
...
// 初始化静态成员
const std::unique_ptr<Wife1> Wife1::wife = std::make_unique<Wife1>();
```
...
class Wife2 {
private:
    static std::unique_ptr<Wife2> wife;
    // 将构造函数私有化，避免外部程序调用创建对象
    Wife2() = default;
public:
    static Wife2& getWife() {
        if(wife == nullptr) {
            wife = std::make_unique<Wife2>();
        }
        return *wife;
    }
};
...
// 初始化静态成员
std::unique_ptr<Wife2> Wife2::wife = nullptr;
```
...
class Wife3 {
private:
    static std::unique_ptr<Wife3> wife;
...
// 初始化静态成员
std::unique_ptr<Wife3> Wife3::wife = nullptr;
std::mutex Wife3::mtx;
```
...
class Wife4 {
private:
    static std::atomic<std::unique_ptr<Wife4>> wife;
    static std::mutex mtx;
    // 将构造函数私有化，避免外部程序调用创建对象
    Wife4() = default;
public:
    static Wife4& getWife() {
        Wife4* tmp = wife.load(std::memory_order_relaxed);
        if(tmp == nullptr) {
            std::lock_guard<std::mutex> lock(mtx);
            tmp = wife.load(std::memory_order_relaxed);
            if(tmp == nullptr) {
                auto new_wife = std::make_unique<Wife4>();
                tmp = new_wife.get();
                wife.store(std::move(new_wife), std::memory_order_relaxed);
            }
        }
        return *tmp;
    }
...
// 初始化静态成员
std::atomic<std::unique_ptr<Wife4>> Wife4::wife{nullptr};
std::mutex Wife4::mtx;
```
...
private:
    ...
    struct WifeHolder {
        static Wife5 instance;
    };
    
public:
    static Wife5& getWife() {
        return WifeHolder::instance;
    }
};

// 初始化静态成员
Wife5 Wife5::WifeHolder::instance{};
```
...
public:
    ...
    static EnumWifeSingleton& getInstance() {
        static EnumWifeSingleton instance;
        return instance;
    }
};
```
...
int main() {
    auto& marry = EnumWifeSingleton::getInstance();
    marry.setName("marry");

    std::cout << "wife's name is :" << marry.getName() << std::endl;
    auto& lily = EnumWifeSingleton::getInstance();
    std::cout << "marry is lily?:" << (&marry == &lily ? "true" : "false") << std::endl;
...
private:
     ...
     * 私有化构造函数
     */
    ContainerRegistrySingleton() = default;
...
public:
    template<typename T>
    static T& getBean(const std::string& className, std::function<std::shared_ptr<T>()> factory = nullptr) {
        std::lock_guard<std::mutex> lock(mtx);
        ...
        // 如果没有提供factory函数，则抛出异常或返回默认值
        throw std::runtime_error("No factory function provided for class: " + className);
    }
};
