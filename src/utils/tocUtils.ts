...
export class TOCManager {
	...
	public generateTOCHTML(): string {
        ...
        <a 
          href="#${heading.id}" 
          class="px-2 flex relative transition w-full min-h-9 rounded-xl hover:bg-[var(--toc-btn-hover)] active:bg-[var(--toc-btn-active)] py-2 ${depthClass}"
          data-heading-id="${heading.id}"
        >
          <div class="transition text-base ${depth <= this.minDepth + 1 ? "text-[#394E6A] dark:text-white" : "text-[#394E6A]/70 dark:text-white"} flex-1 min-w-0 overflow-hidden text-ellipsis whitespace-nowrap">${headingText}</div>
        </a>
      `;
		});
