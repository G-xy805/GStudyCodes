---
title: "设计模式——命令模式"
description: "命令模式是一种行为设计模式，它尝试以对象来代表实际行动。命令对象可以把行动及其参数封装起来，于是这些行动可以被重复多次、取消或重新执行。本文详细介绍了命令模式的基本概念、优缺点、结构以及C++实现方式。"
pubDate: "2024-03-15"
image: "/assets/img/design-patterns/command.webp"
categories: ["设计模式", "行为型模式"]
tags: ["命令模式", "C++", "设计模式", "行为型模式", "封装命令"]
---
...
public:
    Invoker(std::shared_ptr<Command> cmd) : command(cmd) {}
...
public:
    ...
    ConcreteCommand(std::shared_ptr<Receiver> recv) : receiver(recv) {}

    void execute() override {
        std::cout << "命令发送给接收者!" << std::endl;
        receiver->action();
    }
};
```
...
int main() {
    auto cmd = std::make_shared<ConcreteCommand>();
    Invoker invoker(cmd);
    invoker.call();
